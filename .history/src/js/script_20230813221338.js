...
const mainSwiper = new Swiper(".mySwiper", {
  ...
  breakpoints: {
    768: {
      slidesPerView: 1.5, // 768px以下の時に1.5枚表示
      slidesPerView: 4,
      spaceBetween: 40,
      width: 1440,
    },
...
swiperSlides.forEach(slide => {
  if (window.innerWidth <= 768) { // 画面幅が768px以下の場合に適用
    slide.style.width = "280px";   // widthを280pxに設定
    slide.style.marginRight = "24px"; // margin-rightを24pxに設定
  } else {
    slide.style.width = "333px";
    slide.style.marginRight = "40px";
  }
});
このコードにより、画面幅が768px以下の場合には指定された幅とマージンが適用され、それ以上の画面幅では元の設定が適用されるようになります。
